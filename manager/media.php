...
<script type="text/babel">
    ...
    upload = () => {
        ...
        axios.post('media/audio', this.audio_form_data, config)
            .then(res => {
...
class VideoUploader extends React.Component {
    ...
    submit = (e) => {
        ...
        axios.post('media/video', data, config)
            .then(res => {
                this.setState({ videouploader: null });
                $(event.target).val('')
                alertify.success('File Uploaded Please Refresh');
                            ...
                            return (
                                <div className="col-xs-6 col-sm-4 col-md-3 col-lg-2 text-center">
                                    <a data-toggle="tooltip" title={new Date(media.createdAt).toString()}>
                                        <h1>{media.type === 'video' ? <i className="fa fa-file-video-o"></i> : <span>{media.type === 'audio' ? <i className="fa fa-file-audio-o"></i> : <i className="fa fa-file"></i>}</span>}</h1>
                                        {media.name}</a>
                                        <p>{media.blob}</p>
                                </div>
                            )
    ...
    return (
        <div>
            <section className="content-header">
                <h1>Media<small>Explorer</small></h1>
